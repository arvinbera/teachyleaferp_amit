...
<div class="content px-3">
    ...
    <div class="card">
        ...
        {!! Form::model($subjectAssignings, ['route' => ['subjectAssignings.update', $subjectAssignings[0]->class_id], 'method' => 'patch', 'id' => 'myForm']) !!}
                ...
                <div class="form-row">
                    <div class="form-group col-sm-4">
                        ...
                        <select class="form-control" name="class_id">
                            ...
                            <option value="{{$class->id}}" {{$subjectAssignings[0]->class_id == $class->id ? 'selected' : ''}}>{{$class->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>

                @foreach($subjectAssignings as $subjectAssigning)
                    ...
                    <div class="form-row">
                        <div class="form-group col-sm-4">
                            ...
                            <select class="form-control" name="subject_id[]" required>
                                ...
                                <option value="{{$subject->id}}" {{$subjectAssigning->subject_id == $subject->id ? 'selected' : ''}}>{{$subject->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-sm-2">
                            {!! Form::label('full_marks', 'Full Marks:') !!}
                            <input type="text" name="full_marks[]" value="{{$subjectAssigning->full_marks}}" class="form-control" required />
                        </div>
                        <div class="form-group col-sm-2">
                            {!! Form::label('pass_marks', 'Pass Marks:') !!}
                            <input type="text" name="pass_marks[]" value="{{$subjectAssigning->pass_marks}}" class="form-control" required />
                        </div>
                        <div class="form-group col-sm-2">
