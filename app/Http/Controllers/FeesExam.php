...
class FeesExam extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('student_fees_exam.index', $data);
    }
    public function filter(Request $request)
    {
        ...
        $exam = ExamTypes::where('id', $request->exam_type_id)->first()['name'];
        ...
        foreach ($students as $key => $value) {
            $fees_exam = FeesAmounts::where('fees_category_id', 3)->where('class_id', $value->class_id)->get()->first();
            $color = 'success';
            ...
            $html[$key]['tdsource'] .= '<td>' . $fees_exam->amount . '</td>';
            $html[$key]['tdsource'] .= '<td>' . $value['feewaive']['discount'] . '</td>';

            $actual_fees = $fees_exam->amount;
            ...
            $html[$key]['tdsource'] .= '<a class="btn btn-' . $color . '" title="Fees Payment Slip" target="_blank" href=' . route("studentFeePayslip2") . '?class_id=' . $value->class_id . '&student_id=' . $value->student_id . '&exam=\'' . $exam . '\'' . '>Fees Payment Slip</a>';
            $html[$key]['tdsource'] .= '</td>';
        }
    ...
    public function payslip(Request $request)
    {
        ...
        $pdf = PDF::loadView('student_fees_exam.payslip', $data);
